...
export const initPDFEdit = async (req, res) => {
  ...
  console.log(`[Init-Edit] Session initialization request received. courseId: ${courseId || 'none'}, file: ${file ? file.originalname : 'none'}`);
  ...
  try {
    ...
    if (courseId) {
      ...
      console.log(`[Init-Edit] Custom PDF upload detected. Temp path: ${sourcePath}, Name: ${originalName}`);
    } else {
      console.warn("[Init-Edit] Warning: Neither courseId nor custom file was provided.");
    ...
    console.log(`[Init-Edit] PDF editing session successfully initialized. Client URL: ${fileUrl}`);
    ...
    console.error('[Init-Edit] Fatal error during PDF edit session initialization:', err);
    res.status(500).json({ error: 'Server error initializing PDF edit session' });
  }
};
...
export const detectPrefix = async (req, res) => {
  ...
  console.log(`[Detect-Prefix] Received prefix detection request. Image payload size: ${image ? image.length : 0} characters.`);

  if (!image) {
    console.error("[Detect-Prefix] Error: Base64 image payload is missing.");
    return res.status(400).json({ error: 'Base64 image data is required for prefix detection' });
  }
  ...
  try {
    ...
    if (!apiKey || apiKey === 'your_gemini_api_key_here') {
      console.error("[Detect-Prefix] Error: Gemini API key is missing or default.");
      return res.status(550).json({ error: 'Gemini API key is not configured in backend .env' });
    }

    console.log("[Detect-Prefix] Initializing Google Generative AI model 'gemini-3.5-flash'...");
    ...
    console.log("[Detect-Prefix] Slicing image headers and preparing base64 part payload...");
...
`;
    ...
    console.log("[Detect-Prefix] Gemini API response text received:", responseText);

    console.log("[Detect-Prefix] Attempting to parse Gemini response as JSON...");
    let parsedResult = {};
    try {
      parsedResult = JSON.parse(responseText.trim());
      console.log("[Detect-Prefix] Successfully parsed Gemini JSON response.");
    } catch (parseErr) {
      console.error('[Detect-Prefix] JSON Parsing Error on Gemini response:', parseErr);
      return res.status(500).json({ error: 'Gemini did not return structured coordinates JSON. Please try again.' });
    }
...
export const applyWhiteout = async (req, res) => {
  ...
  if (!box || typeof box.x !== 'number' || typeof box.y !== 'number' || typeof box.width !== 'number' || typeof box.height !== 'number') {
    ...
    return res.status(400).json({ error: 'Valid box coordinates {x, y, width, height} are required' });
  }
  if (!viewport || typeof viewport.width !== 'number' || typeof viewport.height !== 'number') {
    console.error("[Apply-Whiteout] Error: Invalid viewport dimensions.");
    return res.status(400).json({ error: 'Valid viewport dimensions {width, height} are required' });
  }
  if (!cleanedText) {
    console.error("[Apply-Whiteout] Error: Missing cleanedText.");
    return res.status(400).json({ error: 'Cleaned text is required to replace the question' });
  }
  ...
  try {
    ...
    try {
      ...
      return res.status(404).json({ error: 'Edited PDF file not found on server.' });
    }
    ...
    console.log(`[Apply-Whiteout] PDF successfully loaded. Page count: ${pages.length}`);

    if (pageNumber > pages.length) {
      console.error(`[Apply-Whiteout] Error: Requested page ${pageNumber} exceeds total pages ${pages.length}`);
      return res.status(400).json({ error: `Page number ${pageNumber} exceeds PDF page count (${pages.length})` });
    }

    const page = pages[pageNumber - 1];
    const { width: pdfWidth, height: pdfHeight } = page.getSize();
    console.log(`[Apply-Whiteout] Selected Page dimensions: ${pdfWidth.toFixed(2)}x${pdfHeight.toFixed(2)}`);

    // Map viewport coordinates to PDF document coordinates
    console.log(`[Apply-Whiteout] Transforming box coordinates from viewport scale (${viewport.width}x${viewport.height}) to PDF scale...`);
    ...
    console.log(`[Apply-Whiteout] Mapped coordinates: x: ${rectX.toFixed(2)}, y: ${rectY.toFixed(2)}, w: ${rectWidth.toFixed(2)}, h: ${rectHeight.toFixed(2)}`);

    // Draw a solid rectangle covering EXACTLY the target selection box area with color #EBEBFF
    console.log("[Apply-Whiteout] Drawing solid replacement rectangle #EBEBFF...");
    page.drawRectangle({
      x: rectX,
      y: rectY,
      width: rectWidth,
    ...
    console.log(`[Apply-Whiteout] Wrapping cleaned text. Max Width: ${maxTextWidth.toFixed(2)}`);
    ...
    console.log(`[Apply-Whiteout] Text alignment layout: xCenter: ${xCenter.toFixed(2)}, yCenter: ${yCenter.toFixed(2)}, startY: ${startY.toFixed(2)}`);
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      console.log(`[Apply-Whiteout] Drawing line ${i+1} at x: ${lineX.toFixed(2)}, y: ${lineY.toFixed(2)}`);
    ...
    console.log(`[Apply-Whiteout] Saving PDF bytes back to disk at: ${filePath}`);
    ...
    res.status(500).json({ error: 'Server error replacing question text in PDF' });
  }
};
...
export const downloadPDF = async (req, res) => {
  ...
  try {
    ...
    console.log(`[Download] Serving file download. Display name: ${displayName}`);
...
export const autoCleanPDF = async (req, res) => {
  const { editId, startPage: startPageParam, maxPages: maxPagesParam } = req.body;
  ...
  const startPageIdx = startPageParam ? Math.max(1, parseInt(startPageParam, 10)) : 1;
  const maxPagesToProcess = maxPagesParam ? Math.max(1, parseInt(maxPagesParam, 10)) : null;
  ...
  try {
    ...
    try {
      ...
      console.error(`[Auto-Clean] Error: File not found at ${filePath}`, accessErr);
      return res.status(404).json({ error: 'Edited PDF file not found on server.' });
    }
    ...
    const pages = pdfDoc.getPages();
    console.log(`[Auto-Clean] PDF loaded successfully. Total pages: ${totalPages}`);

    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey || apiKey === 'your_gemini_api_key_here') {
      console.error("[Auto-Clean] Error: Gemini API key is missing or default.");
      return res.status(550).json({ error: 'Gemini API key is not configured in backend .env' });
    }
    ...
    const limitPages = maxPagesToProcess ? Math.min(totalPages, startPageIdx + maxPagesToProcess - 1) : totalPages;
    console.log(`[Auto-Clean] Starting chunk-based processing loop. startPage: ${startPageIdx}, limitPages: ${limitPages}, totalPages: ${totalPages}, chunk size: ${chunkSize} pages.`);
    
    for (let startPage = startPageIdx; startPage <= limitPages; startPage += chunkSize) {
      ...
      console.log(`[Auto-Clean] Slicing pages ${startPage}-${endPage} into temporary document...`);
      ...
      console.log(`[Auto-Clean] Temporary chunk PDF created. Size: ${chunkBytes.length} bytes.`);
...
`;
      ...
      for (let attempt = 1; attempt <= retries; attempt++) {
        try {
          console.log(`[Auto-Clean] Calling Gemini API (Attempt ${attempt}/${retries})...`);
          ...
          console.error(`[Auto-Clean] Attempt ${attempt} failed:`, err.message || err);
          if (attempt === retries) {
            console.error(`[Auto-Clean] Error: All ${retries} attempts failed calling Gemini API.`);
            throw err; 
          }
          console.log(`[Auto-Clean] Waiting ${delayMs}ms before retrying due to API error...`);
      ...
      try {
        ...
        console.log(`[Auto-Clean] Successfully parsed response JSON. Found ${chunkResults.length} pages to clean.`);
      } catch (parseErr) {
        console.error(`[Auto-Clean] JSON parsing failed for chunk starting at page ${startPage}. Skipping chunk.`, parseErr);
        continue; 
      }

      if (!Array.isArray(chunkResults)) {
        console.error(`[Auto-Clean] Error: Gemini response is not an array. Skipping chunk.`);
        continue;
      }
      ...
      for (const item of chunkResults) {
        ...
        if (!relPageNum || relPageNum < 1 || relPageNum > pageIndices.length) {
          console.warn(`[Auto-Clean] Warning: Invalid relative page number ${relPageNum} returned. Skipping page.`);
          continue;
        }
        ...
        if (!cleanedText || !box || typeof box.ymin !== 'number' || typeof box.xmin !== 'number' || typeof box.ymax !== 'number' || typeof box.xmax !== 'number') {
          console.warn(`[Auto-Clean] Warning: Incomplete data returned for page ${absPageNum}. Skipping.`);
          continue;
        }

        console.log(`[Auto-Clean] Cleaning page ${absPageNum}:`);
        console.log(`  - Original Text: "${item.originalText?.slice(0, 100)}..."`);
        console.log(`  - Cleaned Text:  "${cleanedText.slice(0, 100)}..."`);
        console.log(`  - Prefix:        "${item.prefixText}"`);
        console.log(`  - Bounding Box:  ymin: ${box.ymin}, xmin: ${box.xmin}, ymax: ${box.ymax}, xmax: ${box.xmax}`);
        ...
        console.log(`  - Drawing mask rectangle: x: ${rectX.toFixed(2)}, y: ${rectY.toFixed(2)}, w: ${rectWidth.toFixed(2)}, h: ${rectHeight.toFixed(2)}`);
      ...
      if (cleanedInChunk > 0) {
        console.log(`[Auto-Clean] Saving intermediate progress back to disk...`);
        const modifiedPdfBytes = await pdfDoc.save();
        await fs.writeFile(filePath, modifiedPdfBytes);
        console.log(`[Auto-Clean] Progress saved for chunk ending at page ${endPage}.`);
      } else {
        console.log(`[Auto-Clean] No modifications made in this chunk. Skipping intermediate save.`);
      }

      // Introduce a delay between chunks to avoid hitting API rate limits
      const waitTimeMs = 2500;
      console.log(`[Auto-Clean] Waiting ${waitTimeMs}ms before starting next chunk...`);
    ...
    console.error('[Auto-Clean] Fatal error during automated PDF cleaning:', err);
    res.status(500).json({ error: 'Server error during automated PDF cleaning: ' + (err.message || err) });
  }
};
...
export const cleanPagesPDF = async (req, res) => {
  ...
  console.log(`[Clean-Pages] Selective cleaning request received. editId: ${editId}, pages count: ${pagesArray?.length}`);
  ...
  try {
    ...
    const totalPages = pdfDoc.getPageCount();
    const pages = pdfDoc.getPages();
    console.log(`[Clean-Pages] PDF successfully loaded. Page count: ${totalPages}`);
    ...
    if (validPages.length === 0) {
      console.error("[Clean-Pages] Error: No valid page numbers provided within PDF range.");
      return res.status(400).json({ error: `No valid page numbers provided within PDF range (1-${totalPages}).` });
    }
    ...
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey || apiKey === 'your_gemini_api_key_here') {
      console.error("[Clean-Pages] Error: Gemini API key is missing or default.");
      return res.status(550).json({ error: 'Gemini API key is not configured in backend .env' });
    }
    ...
    for (let i = 0; i < validPages.length; i += chunkSize) {
      ...
      // Create a temporary document containing only the chunk pages
      console.log(`[Clean-Pages] Slicing pages [${chunkPages.join(', ')}] into temporary document...`);
      const chunkDoc = await PDFDocument.create();
      ...
      copiedPages.forEach(p => chunkDoc.addPage(p));
      const chunkBytes = await chunkDoc.save();
      console.log(`[Clean-Pages] Temporary chunk PDF created. Size: ${chunkBytes.length} bytes.`);
...
`;
      ...
      for (let attempt = 1; attempt <= retries; attempt++) {
        try {
          console.log(`[Clean-Pages] Calling Gemini API (Attempt ${attempt}/${retries})...`);
          const response = await model.generateContent([prompt, pdfPart]);
          responseText = response.response.text();
          geminiSuccess = true;
          ...
          console.error(`[Clean-Pages] Attempt ${attempt} failed:`, err.message || err);
          if (attempt === retries) {
            console.error(`[Clean-Pages] Error: All ${retries} attempts failed calling Gemini API.`);
            throw err; 
          }
          console.log(`[Clean-Pages] Waiting ${delayMs}ms before retrying due to API error...`);
          await new Promise(resolve => setTimeout(resolve, delayMs));
          delayMs *= 2; // Exponential backoff
        }
      }
      ...
      try {
        chunkResults = JSON.parse(responseText.trim());
        console.log(`[Clean-Pages] Successfully parsed response JSON. Found ${chunkResults.length} pages to clean.`);
      } catch (parseErr) {
        console.error(`[Clean-Pages] JSON parsing failed for chunk. Skipping chunk.`, parseErr);
        continue; 
      }

      if (!Array.isArray(chunkResults)) {
        console.error(`[Clean-Pages] Error: Gemini response is not an array. Skipping chunk.`);
        continue;
      }
      ...
      for (const item of chunkResults) {
        ...
        if (!relPageNum || relPageNum < 1 || relPageNum > chunkPages.length) {
          console.warn(`[Clean-Pages] Warning: Invalid relative page number ${relPageNum} returned. Skipping page.`);
          continue;
        }
        ...
        if (!cleanedText || !box || typeof box.ymin !== 'number' || typeof box.xmin !== 'number' || typeof box.ymax !== 'number' || typeof box.xmax !== 'number') {
          console.warn(`[Clean-Pages] Warning: Incomplete data returned for page ${absPageNum}. Skipping.`);
          continue;
        }
      ...
      // Save document after each successfully processed chunk to preserve progress
      if (cleanedInChunk > 0) {
        console.log(`[Clean-Pages] Saving intermediate progress back to disk...`);
        const modifiedPdfBytes = await pdfDoc.save();
        await fs.writeFile(filePath, modifiedPdfBytes);
        console.log(`[Clean-Pages] Progress saved for chunk.`);
      } else {
        console.log(`[Clean-Pages] No modifications made in this chunk. Skipping intermediate save.`);
      }

      // Introduce a delay between chunks to avoid hitting API rate limits
      if (i + chunkSize < validPages.length) {
        const waitTimeMs = 2500;
        console.log(`[Clean-Pages] Waiting ${waitTimeMs}ms before starting next chunk...`);
        await new Promise(resolve => setTimeout(resolve, waitTimeMs));
      }
    }

    console.log(`[Clean-Pages] Specific page cleaning complete for file: ${filePath}! Total cleaned: ${totalCleanedCount}`);
    res.json({
    ...
    res.status(500).json({ error: 'Server error during PDF cleaning: ' + (err.message || err) });
  }
};
