...
export const uploadQuestionPaper = async (req, res) => {
  ...
  console.log("File Info - Name:", file?.originalname, "| Size:", file?.size, "bytes");
  console.log("======================================================\n");
  ...
  try {
    ...
    try {
      ...
      console.error("PDFParse error (falling back to multimodal PDF scan):", parseErr);
    }

    if (!pdfText || pdfText.trim().length < 200) {
      console.log("PDF text is extremely short or empty. Treating as a SCANNED/IMAGE-based PDF.");
      isScannedOrEmpty = true;
    } else {
      console.log("PDF text extracted successfully. Treating as a DIGITAL PDF.");
    }
    ...
    } else if (fullSyllabus.optionalSubjects && fullSyllabus.optionalSubjects[subject]) {
      subjectSyllabus = fullSyllabus.optionalSubjects[subject];
      subjectDisplayName = `Optional subject: ${subject.replace('OptionalSubject', '')}`;
    }

    console.log("Step 5: Matching subject syllabus outline. Display Name:", subjectDisplayName);
    if (!subjectSyllabus) {
      console.error("Syllabus Error: Syllabus outline not found for", subject);
      return res.status(400).json({ error: `Syllabus not found for subject: ${subject}` });
    }
    ...
    console.log("Simplified syllabus outline sections loaded:", syllabusOutline.length);
    ...
    if (!apiKey || apiKey === 'your_gemini_api_key_here') {
      ...
      return res.status(550).json({ error: 'Gemini API key is not configured in backend .env file.' });
    }
...
`;
    ...
    let promptParts = [];
    if (isScannedOrEmpty) {
      console.log("Step 7: Preparing raw PDF binary for Gemini multimodal OCR...");
      const pdfPart = {
        inlineData: {
    ...
    try {
      ...
      console.log("Parsed JSON successfully. Extracted question array count:", extractedQuestions.length);
    } catch (parseErr) {
      console.error('Error parsing Gemini JSON response:', parseErr);
      return res.status(500).json({ error: 'Failed to parse structured JSON from Gemini. Please try again.' });
    }

    if (!Array.isArray(extractedQuestions)) {
      console.error("Format Error: Response is not a JSON array");
      return res.status(500).json({ error: 'Gemini did not return a valid list of questions.' });
    }
    ...
    for (let i = 0; i < extractedQuestions.length; i++) {
      ...
      console.log(`Saving question ${i+1}/${extractedQuestions.length}: "${q.text.slice(0, 60)}..."`);
    ...
    console.log("Step 11: All database writes complete. Total questions saved:", savedQuestions.length);
    console.log("======================================================\n");
